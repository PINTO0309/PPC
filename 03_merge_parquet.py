...
def validate_ppc_dataframe(
    ...
    *,
    ...
    if missing_columns:
        raise ValueError(f"PPC parquet {source} is missing required columns: {missing_columns}")
    if df.empty:
        if require_both_classes:
            raise ValueError(f"PPC parquet {source} is empty.")
        return
    if df[["class_id", "label"]].isna().any().any():
        raise ValueError(f"PPC parquet {source} contains null class_id or label values.")

    class_ids = {int(value) for value in df["class_id"]}
    invalid_ids = sorted(class_ids - set(LABEL_MAP))
    if invalid_ids:
        raise ValueError(
            f"PPC parquet {source} contains unsupported class_id values {invalid_ids}; only 0 and 1 are allowed."
        )
    for row_index, row in df[["class_id", "label"]].iterrows():
        ...
        if str(row["label"]).strip() != expected_label:
            raise ValueError(
                f"PPC label/class_id mismatch in {source} at row {row_index}: class_id={class_id} "
                f"requires label={expected_label!r}, got {row['label']!r}."
            )
    if require_both_classes and class_ids != set(LABEL_MAP):
        missing_ids = sorted(set(LABEL_MAP) - class_ids)
        raise ValueError(f"Merged PPC dataset must contain both classes; missing class_id values {missing_ids}.")
    if require_both_classes and "split" in df.columns:
        ...
        if train_ids != set(LABEL_MAP):
            ...
            raise ValueError(
                f"Merged PPC training split must contain both classes; missing class_id values {missing_ids}."
            )
...
def merge_parquet_files(input_paths: list[Path]) -> pd.DataFrame:
    ...
    for path in input_paths:
        ...
        validate_ppc_dataframe(df, source=path, require_both_classes=False)
        if df.empty:
    ...
    validate_ppc_dataframe(merged, source="merged dataset", require_both_classes=True)
    return merged
...
def save_class_ratio_chart(class_counts: Counter[int], output_path: Path) -> None:
    ...
    if total == 0:
        ...
        return
    labels = [
        f"{LABEL_MAP[class_id]} (class_id={class_id})"
