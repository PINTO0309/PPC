#!/usr/bin/env python3
"""Export Sigmoid->Mul feature maps from PPC ONNX models as heatmap PNG images."""
...
CLASS_LABELS = ("no_possession", "possession")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a PPC ONNX model, capture Sigmoid->Mul feature maps, and save them as heatmap PNGs."
        )
    )
    parser.add_argument("--model", default="ppc_s_48x48.onnx", help="Path to the ONNX model to inspect.")
...
def _load_model(model_path: Path) -> onnx.ModelProto:
    ...
    if dims and dims[-1].HasField("dim_value") and dims[-1].dim_value != len(CLASS_LABELS):
        raise ValueError(
            ...
            f"but {model_path} declares shape dimension {dims[-1].dim_value}."
        )
    return model


def _validate_ppc_probabilities(output: np.ndarray, model_path: Path) -> np.ndarray:
    raw_output = np.asarray(output)
    if raw_output.shape == (1, len(CLASS_LABELS)):
        probabilities = raw_output[0]
    elif raw_output.shape == (len(CLASS_LABELS),):
        probabilities = raw_output
    else:
        raise ValueError(
            f"PPC ONNX output must contain {len(CLASS_LABELS)} probabilities in "
            f"[no_possession, possession] order; got shape {raw_output.shape} from {model_path}."
        )
    if not np.all(np.isfinite(probabilities)):
        raise ValueError(f"PPC ONNX output contains non-finite values: {probabilities!r}")
    if np.any(probabilities < 0.0) or np.any(probabilities > 1.0):
        raise ValueError(f"PPC ONNX output contains values outside [0, 1]: {probabilities!r}")
    if not np.isclose(float(probabilities.sum()), 1.0, atol=1e-4):
        raise ValueError(f"PPC ONNX probabilities must sum to 1, got {float(probabilities.sum()):.6f}.")
...
def main() -> None:
    ...
    prob = _validate_ppc_probabilities(session.run([final_output_name], feeds)[0], model_path)
    parts = [f"prob_{label}={float(prob[idx]):.4f}" for idx, label in enumerate(CLASS_LABELS)]
