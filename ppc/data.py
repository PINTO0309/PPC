...
def _normalize_label_name(value: str) -> str:
    """Normalize label text by lowercasing and collapsing separators."""
...
class Sample:
    """Container describing a single phone-possession frame."""
...
def _to_label(row: pd.Series) -> int:
    ...
    if "class_id" in row and pd.notna(row["class_id"]):
        ...
        if label_text:
            expected = CLASS_ID_TO_NAME[label_id]
            if label_text != expected:
                raise ValueError(
                    f"Label/class_id mismatch: class_id={label_id} requires label={expected!r}, "
                    f"but got {label_text!r}."
                )
        return label_id
    if label_text:
        if label_text in CLASS_NAME_TO_ID:
            return CLASS_NAME_TO_ID[label_text]
...
def split_samples(
    ...
    for split, subset in assignments.items():
        if not subset:
            continue
        total = len(subset)
        class_counts = Counter(sample.label for sample in subset)
        distribution = ", ".join(
            f"{CLASS_ID_TO_NAME[idx]}={class_counts.get(idx, 0)}" for idx in sorted(CLASS_ID_TO_NAME)
        )
        logger.info(
            "Split '%s': %d samples from %d sources (%s).",
            split,
            total,
    ...
    train_labels = {sample.label for sample in assignments["train"]}
    expected_labels = set(CLASS_ID_TO_NAME)
    if train_labels != expected_labels:
        missing = sorted(expected_labels - train_labels)
        unexpected = sorted(train_labels - expected_labels)
        details = []
        if missing:
            details.append(f"missing class_id values {missing}")
        if unexpected:
            details.append(f"unexpected class_id values {unexpected}")
        raise ValueError(f"Training split must contain both PPC classes ({'; '.join(details)}).")

    return assignments


class PPCDataset(Dataset):
    """Torch dataset that reads phone-possession crops from disk or embedded bytes."""
