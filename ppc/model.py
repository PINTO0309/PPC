...
@dataclass
class ModelConfig:
    """Lightweight CNN configuration for the PPC classifier."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.num_classes != 2:
            raise ValueError(
                f"PPC requires exactly 2 output classes, but num_classes={self.num_classes} was requested."
            )
...
class PPC(nn.Module):
    """Compact phone-possession classifier that outputs two logits."""

    def __init__(self, config: Optional[ModelConfig] = None) -> None:
        super().__init__()
        self.config = config or ModelConfig()
        self.num_classes = int(getattr(self.config, "num_classes", 2))
        if self.num_classes != 2:
            raise ValueError(
                f"PPC requires exactly 2 output classes, but num_classes={self.num_classes} was requested."
            )
