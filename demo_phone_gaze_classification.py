...
DEFAULT_PUC_MODEL = 'puc_l_48x48.onnx'
DEFAULT_PUC_INPUT_SIZE = (48, 48)
DEFAULT_PPC_MODEL = 'ppc_l_48x48.onnx'
DEFAULT_PPC_INPUT_SIZE = (48, 48)
DEFAULT_GAZELLE_MODEL = 'gazelle_hgnetv2_pico_inout_distill_1x3x640x640_1xNx4.onnx'
DEFAULT_GAZE_HAND_OVERLAP_RATIO = 0.6
...
class Box():
    ...
    gaze_hand_overlap: bool = False
    gaze_inout_score: float = -1.0
    gaze_inout_pass: bool = True
...
def finalize_phone_usage_current_frame(
    body_box: Box,
    *,
    enable_puc_gate: bool = True,
) -> None:
    """Apply the current-frame gaze-gated PUC result without temporal smoothing."""
    if apply_ppc_suppression_to_body(body_box):
        return
    is_active = (
        (not enable_puc_gate or (body_box.phone_class is not None and body_box.phone_class > 0))
        and body_box.phone_state == 1
        and body_box.gaze_inout_pass
    )
    body_box.phone_state = 1 if is_active else 0
    body_box.phone_label = PHONE_CHECK_LABEL if is_active else ''
        ...
        *,
        ...
        if self._runtime == 'onnx':
            ...
            if len(inputs) != 1 or len(inputs[0].shape) != 4:
                raise ValueError(
                    'WholeBody detector must expose one rank-4 image input; '
                    f'got {[input.shape for input in inputs]}.'
                )
            if inputs[0].shape[0] != 1 or inputs[0].shape[1] != 3:
                raise ValueError(
                    f'WholeBody detector image input must have shape [1, 3, H, W]; got {inputs[0].shape}.'
                )
            if len(outputs) != 1 or len(outputs[0].shape) != 3:
                raise ValueError(
                    'WholeBody detector must expose one rank-3 prediction output; '
...
class GazelleHeatmapONNX(AbstractModel):
    """ONNXRuntime wrapper for batched Gazelle gaze heatmap inference."""

    def __init__(
        self,
        *,
        model_path: str = DEFAULT_GAZELLE_MODEL,
        providers: Optional[List] = None,
    ) -> None:
        if not model_path:
            raise ValueError('Gazelle model path is empty.')
        if not os.path.isfile(model_path):
            raise FileNotFoundError(f'Gazelle ONNX model not found: {model_path}')
        ...
        if len(inputs) != 2:
            raise ValueError(
                f'Gazelle must expose exactly two inputs (image and head bboxes); got {len(inputs)}.'
            )
        if len(inputs[0].shape) != 4:
            raise ValueError(f'Gazelle image input must be rank 4; got {inputs[0].shape}.')
        if inputs[0].shape[0] != 1 or inputs[0].shape[1] != 3:
            raise ValueError(f'Gazelle image input must have shape [1, 3, H, W]; got {inputs[0].shape}.')
        if len(inputs[1].shape) != 3 or inputs[1].shape[-1] != 4:
            raise ValueError(f'Gazelle bbox input must have shape [1, N, 4]; got {inputs[1].shape}.')
        if inputs[1].shape[0] != 1:
            raise ValueError(f'Gazelle bbox input batch size must be one; got {inputs[1].shape}.')
        if not outputs:
            raise ValueError('Gazelle does not expose any outputs.')

        self._input_height = self._require_static_dimension(inputs[0].shape[2], 'image height')
        self._input_width = self._require_static_dimension(inputs[0].shape[3], 'image width')
        self._heatmap_output_index = (
            self._output_names.index('heatmap')
            if 'heatmap' in self._output_names
            else 0
        )
        self._inout_output_index: Optional[int] = (
            self._output_names.index('inout')
            if 'inout' in self._output_names
            else None
        )
        heatmap_shape = outputs[self._heatmap_output_index].shape
        if len(heatmap_shape) != 3:
            raise ValueError(f'Gazelle heatmap output must have shape [N, H, W]; got {heatmap_shape}.')
        if self._inout_output_index is not None:
            inout_shape = outputs[self._inout_output_index].shape
            if len(inout_shape) != 1:
                raise ValueError(f'Gazelle inout output must have shape [N]; got {inout_shape}.')

    def has_inout_output(self) -> bool:
        return self._inout_output_index is not None

    @staticmethod
    def _require_static_dimension(value: Any, label: str) -> int:
        try:
            dimension = int(value)
        except (TypeError, ValueError) as exc:
            raise ValueError(f'Gazelle {label} must be a static positive integer; got {value!r}.') from exc
        if dimension <= 0:
            raise ValueError(f'Gazelle {label} must be positive; got {dimension}.')
        return dimension
    ...
    def __call__(
        ...
        *,
        image: np.ndarray,
        head_boxes: List[Box],
    ) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], List[Box]]:
        if image is None or image.size == 0:
            raise ValueError('Input image for Gazelle is empty.')
        if not head_boxes:
            return None, None, []

        frame_height, frame_width = image.shape[:2]
        if frame_height <= 0 or frame_width <= 0:
            raise ValueError('Input image for Gazelle has invalid dimensions.')

        normalized_boxes: List[List[float]] = []
        valid_boxes: List[Box] = []
        inv_width = 1.0 / float(frame_width)
        inv_height = 1.0 / float(frame_height)
        for box in head_boxes:
            x1 = float(np.clip(box.x1 * inv_width, 0.0, 1.0))
            y1 = float(np.clip(box.y1 * inv_height, 0.0, 1.0))
            x2 = float(np.clip(box.x2 * inv_width, 0.0, 1.0))
            y2 = float(np.clip(box.y2 * inv_height, 0.0, 1.0))
            if x2 <= x1 or y2 <= y1:
                continue
        ...
        image_input = np.asarray([image_tensor], dtype=self._input_dtypes[0])
        bbox_input = np.asarray([normalized_boxes], dtype=self._input_dtypes[1])
        outputs = super().__call__(input_datas=[image_input, bbox_input])
        if not outputs:
            raise RuntimeError('Gazelle returned no outputs.')

        heatmaps = np.asarray(outputs[self._heatmap_output_index], dtype=np.float32)
        if heatmaps.ndim != 3:
            raise RuntimeError(f'Gazelle returned an invalid heatmap shape: {heatmaps.shape}.')
        if heatmaps.shape[0] != len(valid_boxes):
            raise RuntimeError(
                'Gazelle heatmap count does not match the number of valid head boxes: '
                f'{heatmaps.shape[0]} != {len(valid_boxes)}.'
            )
        heatmaps = np.nan_to_num(heatmaps, nan=0.0, posinf=1.0, neginf=0.0)
        heatmaps = np.clip(heatmaps, 0.0, 1.0)

        inout_scores: Optional[np.ndarray] = None
        if self._inout_output_index is not None:
            inout_scores = np.asarray(outputs[self._inout_output_index], dtype=np.float32)
            if inout_scores.ndim != 1:
                raise RuntimeError(f'Gazelle returned an invalid inout shape: {inout_scores.shape}.')
            if inout_scores.shape[0] != len(valid_boxes):
                raise RuntimeError(
                    'Gazelle inout count does not match the number of valid head boxes: '
                    f'{inout_scores.shape[0]} != {len(valid_boxes)}.'
                )
            inout_scores = np.nan_to_num(inout_scores, nan=0.0, posinf=1.0, neginf=0.0)
            inout_scores = np.clip(inout_scores, 0.0, 1.0)
    ...
    def _preprocess(
        ...
        *,
        ...
        if image.ndim != 3 or image.shape[2] != 3:
            raise ValueError('Gazelle expects a BGR image with shape [H, W, 3].')
        resized = cv2.resize(
            image,
...
def gaze_inout_score_passes(
    inout_score: Optional[float],
    *,
    has_inout_output: bool,
    threshold: float = GAZE_INOUT_THRESHOLD,
) -> bool:
    """Apply the in-frame gate while preserving models without an inout head."""
    if not 0.0 <= threshold <= 1.0:
        raise ValueError('threshold must be in the range [0.0, 1.0].')
    if not has_inout_output:
        return True
    if inout_score is None or not np.isfinite(inout_score):
        return False
    return float(inout_score) >= threshold

def assign_boxes_to_bodies(
    target_boxes: List[Box],
    body_boxes: List[Box],
) -> Dict[int, List[Box]]:
    """Assign boxes by center containment, using nearest body center as a tie-breaker."""
    boxes_by_body: Dict[int, List[Box]] = {}
    for target_box in target_boxes:
        target_box.person_id = -1
        candidates = [
            (body_index, body_box)
            for body_index, body_box in enumerate(body_boxes)
            if body_box.x1 <= target_box.cx <= body_box.x2
            and body_box.y1 <= target_box.cy <= body_box.y2
        ]
        if not candidates:
            continue
        body_index, _ = min(
            ...
            key=lambda candidate: (
                (candidate[1].cx - target_box.cx) ** 2
                + (candidate[1].cy - target_box.cy) ** 2
            ),
        )
        target_box.person_id = body_index
        boxes_by_body.setdefault(body_index, []).append(target_box)
    return boxes_by_body

def calculate_expanded_box_bounds(
    target_box: Box,
    *,
    frame_height: int,
    frame_width: int,
    ...
    """Calculate the clipped bounds used for a center-expanded PUC crop."""
    if expansion <= 0:
        raise ValueError('expansion must be positive.')
    if frame_height <= 0 or frame_width <= 0:
        return None

    box_width = float(target_box.x2 - target_box.x1)
    box_height = float(target_box.y2 - target_box.y1)
    if box_width <= 0 or box_height <= 0:
        return None

    center_x = (float(target_box.x1) + float(target_box.x2)) / 2.0
    center_y = (float(target_box.y1) + float(target_box.y2)) / 2.0
...
def gaze_heatmap_overlaps_box(
    ...
    *,
    ...
    """Return whether an expanded PUC crop contains a near-global heatmap peak."""
    ...
    if heatmap_array.ndim != 2 or heatmap_array.size == 0:
        return False
    heatmap_array = np.nan_to_num(heatmap_array, nan=0.0, posinf=1.0, neginf=0.0)
    heatmap_array = np.clip(heatmap_array, 0.0, 1.0)
    global_peak = float(np.max(heatmap_array))
    if global_peak <= np.finfo(np.float32).eps:
        return False

    expanded_bounds = calculate_expanded_box_bounds(
        target_box,
    ...
    box_x1, box_y1, box_x2, box_y2 = expanded_bounds

    map_x1 = int(np.floor(box_x1 / frame_width * heatmap_width))
    map_y1 = int(np.floor(box_y1 / frame_height * heatmap_height))
    map_x2 = int(np.ceil(box_x2 / frame_width * heatmap_width))
    map_y2 = int(np.ceil(box_y2 / frame_height * heatmap_height))
    map_x1 = int(np.clip(map_x1, 0, heatmap_width))
    map_y1 = int(np.clip(map_y1, 0, heatmap_height))
    map_x2 = int(np.clip(map_x2, 0, heatmap_width))
    map_y2 = int(np.clip(map_y2, 0, heatmap_height))
    if map_x2 <= map_x1 or map_y2 <= map_y1:
        return False

    roi_peak = float(np.max(heatmap_array[map_y1:map_y2, map_x1:map_x2]))
    return roi_peak >= global_peak * overlap_ratio
...
def overlay_gaze_heatmaps(
    ...
    *,
    ...
    """Max-compose candidate heatmaps and resize only the aggregate for display."""
    if heatmaps is None or not heatmaps.size or alpha <= 0.0:
        return base_image
    if not 0.0 <= alpha <= 1.0:
        raise ValueError('alpha must be in the range [0.0, 1.0].')

    aggregated = np.max(np.asarray(heatmaps, dtype=np.float32), axis=0)
    ...
    return cv2.addWeighted(base_image, 1.0, heatmap_color, alpha, 0.0)
...
def positive_puc_hands_by_body(
    hands_by_body: Dict[int, List[Box]],
    body_boxes: List[Box],
    *,
    enable_puc_gate: bool = True,
) -> Dict[int, List[Box]]:
    """Return PPC-approved hands, optionally requiring a positive PUC action."""
    return {
        body_index: eligible_hands
        for body_index, hand_boxes in hands_by_body.items()
        if body_index < len(body_boxes) and not body_boxes[body_index].puc_forced_no_action
        if (
            eligible_hands := [
                hand_box
                for hand_box in hand_boxes
                if not enable_puc_gate or hand_box.phone_class in (1, 2)
            ]
        )
    }
...
def main():
    ...
    def check_unit_interval(value):
        ...
        if fvalue < 0.0 or fvalue > 1.0:
            raise ArgumentTypeError(
                f"Invalid Value: {fvalue}. Please specify a float in the range [0.0, 1.0]."
            )
        return fvalue
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=(
            'Enable the PUC action gate. When omitted, all PPC-approved Hands are eligible '
            'for gaze classification regardless of the PUC class.'
        ),
    )
    parser.add_argument(
        '--ppc_model',
    ...
    parser.add_argument(
        ...
        '--gazelle-model',
        type=str,
        default=DEFAULT_GAZELLE_MODEL,
        help='ONNX file path for Gazelle heatmap-only or heatmap+inout gaze estimation.',
    )
    ...
    parser.add_argument(
        '--gaze-heatmap-alpha',
        type=check_unit_interval,
        default=DEFAULT_GAZE_HEATMAP_ALPHA,
        help=f'Gazelle heatmap overlay alpha. Default: {DEFAULT_GAZE_HEATMAP_ALPHA}',
    )
    heatmap_render_group = parser.add_mutually_exclusive_group()
    heatmap_render_group.add_argument(
        '--enable-heatmap',
        action='store_true',
        help='Render Gazelle heatmaps. Gaze classification remains enabled without this flag.',
    )
    heatmap_render_group.add_argument(
        '--disable-gaze-heatmap-overlay',
        dest='enable_heatmap',
        action='store_false',
        help=SUPPRESS,
    )
    parser.set_defaults(enable_heatmap=False)
    parser.add_argument(
    ...
    enable_puc_gate = args.enable_puc
    ...
    gaze_hand_overlap_ratio = args.gaze_hand_overlap_ratio
    gaze_heatmap_alpha = args.gaze_heatmap_alpha
    enable_gaze_heatmap_overlay = args.enable_heatmap
    ...
    required_model_files = {
        'WholeBody detector': model_file,
        'PUC classifier': puc_model_file,
        'PPC classifier': ppc_model_file,
        'Gazelle gaze estimator': gazelle_model_file,
    }
    for model_label, required_model_file in required_model_files.items():
        if not required_model_file or not os.path.isfile(required_model_file):
            parser.error(f'{model_label} model not found: {required_model_file}')
    model_dir_path = os.path.dirname(os.path.abspath(model_file))
    model_ext: str = os.path.splitext(model_file)[1][1:].lower()
    if os.path.splitext(puc_model_file)[1].lower() != '.onnx':
        parser.error(f'PUC classifier must be an ONNX model: {puc_model_file}')
    if os.path.splitext(ppc_model_file)[1].lower() != '.onnx':
        parser.error(f'PPC classifier must be an ONNX model: {ppc_model_file}')
    if os.path.splitext(gazelle_model_file)[1].lower() != '.onnx':
        parser.error(f'Gazelle gaze estimator must be an ONNX model: {gazelle_model_file}')
    if not is_package_installed('onnxruntime'):
        parser.error('onnxruntime is required for the PUC, PPC, and Gazelle ONNX models.')
    runtime: str = None
    if model_ext == 'onnx':
        runtime = 'onnx'
        ...
        parser.error(f'Unsupported WholeBody detector model format: {model_file}')
    ...
    gazelle_heatmap_model = GazelleHeatmapONNX(
        model_path=gazelle_model_file,
        providers=providers,
    )
    gazelle_has_inout_output = gazelle_heatmap_model.has_inout_output()
    ...
    while True:
        ...
        hands_by_body = assign_boxes_to_bodies(phone_usage_target_boxes, body_boxes)
        heads_by_body = assign_boxes_to_bodies(head_boxes, body_boxes)
        positive_hands_by_body = positive_puc_hands_by_body(
            hands_by_body,
            body_boxes,
            enable_puc_gate=enable_puc_gate,
        )

        for body_box in body_boxes:
            body_box.gaze_hand_overlap = False
            body_box.gaze_inout_score = -1.0
            body_box.gaze_inout_pass = not gazelle_has_inout_output
            body_box.phone_state = 0
            body_box.phone_label = ''

        gazelle_head_boxes: List[Box] = []
        gazelle_body_index_by_head_id: Dict[int, int] = {}
        for body_index in sorted(positive_hands_by_body):
            candidate_heads = heads_by_body.get(body_index, [])
            if not candidate_heads:
                continue
            selected_head = max(candidate_heads, key=lambda head_box: head_box.score)
            gazelle_head_boxes.append(selected_head)
            gazelle_body_index_by_head_id[id(selected_head)] = body_index

        gaze_heatmaps: Optional[np.ndarray] = None
        gaze_inout_scores: Optional[np.ndarray] = None
        if gazelle_head_boxes:
            gaze_heatmaps, gaze_inout_scores, valid_gazelle_heads = gazelle_heatmap_model(
                image=image,
                head_boxes=gazelle_head_boxes,
            )
            if gaze_heatmaps is not None:
                for gaze_index, (heatmap, head_box) in enumerate(
                    zip(gaze_heatmaps, valid_gazelle_heads)
                ):
                    body_index = gazelle_body_index_by_head_id.get(id(head_box))
                    if body_index is None:
                        continue
                    gaze_hand_overlap = any(
                        gaze_heatmap_overlaps_box(
                            heatmap,
                            hand_box,
                    ...
                    body_box = body_boxes[body_index]
                    inout_score = (
                        float(gaze_inout_scores[gaze_index])
                        if gaze_inout_scores is not None
                        else None
                    )
                    inout_pass = gaze_inout_score_passes(
                        inout_score,
                        has_inout_output=gazelle_has_inout_output,
                    )
                    body_box.gaze_hand_overlap = gaze_hand_overlap
                    body_box.gaze_inout_score = (
                        inout_score
                        if inout_score is not None
                        else -1.0
                    )
                    body_box.gaze_inout_pass = inout_pass
                    body_box.phone_state = 1 if gaze_hand_overlap and inout_pass else 0

        if enable_gaze_heatmap_overlay:
            debug_image = overlay_gaze_heatmaps(
                debug_image,
                gaze_heatmaps,
                alpha=gaze_heatmap_alpha,
            )
        elapsed_time = time.perf_counter() - start_time

        for body_box in body_boxes:
            finalize_phone_usage_current_frame(
                body_box,
                enable_puc_gate=enable_puc_gate,
            )
        ...
        for box in boxes:
            ...
            if classid == 0:
                if phone_label_active:
                    attr_txt = f'{PHONE_CHECK_LABEL} {box.phone_confidence:.3f}'
