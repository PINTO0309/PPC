#!/usr/bin/env python3
"""Estimate feature importance for PPC ONNX models by ablating intermediate tensors."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Insert scalar gates after selected ONNX tensors and measure target PPC probability drops when zeroed."
    )
    parser.add_argument("--model", default="ppc_s_48x48.onnx", help="Path to ONNX model.")
    ...
    parser.add_argument(
        ...
        default="possession",
        help=(
            "Class probability to score: no_possession/0 or possession/1. Default: possession."
        ),
    )
    ...
    default_heatmap_script = Path(__file__).with_name("10_visualize_ppc_heatmaps.py")
    parser.add_argument(
        "--heatmap-script",
        default=str(default_heatmap_script),
        help="Path to 10_visualize_ppc_heatmaps.py used for auto-rendering top-K heatmaps.",
    )
...
def _parse_target_score(value: str) -> TargetScoreSpec:
    ...
    if normalized in TARGET_CLASS_ALIASES:
        ...
        if index not in CLASS_LABELS:
            raise argparse.ArgumentTypeError("PPC class index must be 0 or 1.")
        indices = (index,)
    label = CLASS_LABELS[indices[0]]
...
def _load_model(path: Path) -> onnx.ModelProto:
    ...
    if dims and dims[-1].HasField("dim_value") and dims[-1].dim_value != len(CLASS_LABELS):
        raise ValueError(
            f"PPC ONNX output must contain {len(CLASS_LABELS)} probabilities, "
            f"but {path} declares shape dimension {dims[-1].dim_value}."
        )
    return model
...
def _run_target_score(
    ...
    if probs.shape == (1, len(CLASS_LABELS)):
        class_probs = probs[0]
    elif probs.shape == (len(CLASS_LABELS),):
        class_probs = probs
    else:
        raise ValueError(
            f"PPC output {output_name!r} must contain exactly {len(CLASS_LABELS)} probabilities in "
            f"[no_possession, possession] order, got shape {probs.shape}."
        )
    if not np.all(np.isfinite(class_probs)):
        raise ValueError(f"PPC output {output_name!r} contains non-finite values: {class_probs!r}")
    if np.any(class_probs < 0.0) or np.any(class_probs > 1.0):
        raise ValueError(f"PPC output {output_name!r} contains values outside [0, 1]: {class_probs!r}")
    if not np.isclose(float(class_probs.sum()), 1.0, atol=1e-4):
        raise ValueError(
            f"PPC output {output_name!r} probabilities must sum to 1, got {float(class_probs.sum()):.6f}."
        )
    return float(class_probs[target.indices[0]])
